...
/*   Updated: 2025/01/31 13:45:23 by luferna3         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

//lst_delone: function to delete a node from the list
#include "libft.h"

void	ft_lstdelone(t_list *lst, void (*del)(void*))
{
	if (!del)//check for validity of del function
	{
		return ;
	}
	del(lst->content);//deletes content at node
	free(lst);//frees memory for the node
}
